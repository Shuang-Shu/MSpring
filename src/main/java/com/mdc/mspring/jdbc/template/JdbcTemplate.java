...
import com.mdc.mspring.utils.ResultSetUtils;
        ...
        return execute(
                ...
                (PreparedStatement ps) -> {
                    ...
                    try (ResultSet rs = ps.executeQuery()) {
                        while (rs.next()) {
                            try {
                                result.add(ResultSetUtils.constructAndSetFieldsUseResultSet(clazz, rs));
                            } catch (Exception e) {
                                throw new DataAccessException(e.getMessage());
                            }
                        }
        ...
        return execute(
                ...
                (PreparedStatement ps) -> {
                    ...
                    try (ResultSet rs = ps.executeQuery()) {
                        if (rs.next()) {
                            try {
                                result = ResultSetUtils.constructAndSetFieldsUseResultSet(clazz, rs);
                            } catch (Exception e) {
                                throw new DataAccessException(e.getMessage());
                            }
                        }
